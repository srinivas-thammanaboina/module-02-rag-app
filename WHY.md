...
- **Pure top-k can't answer cross-company comparisons** (retrieval Finding 2). "How do Tesla and NVIDIA describe their AI investments?" returns 5 NVIDIA chunks and 0 Tesla, because NVIDIA's AI prose embeds harder against the query. No prompt can fix this — the generator can only ground in what it's shown. The structural cure is per-company round-robin retrieval (Experiment 7) *upstream* of generation — **now the outer layer of the shipped `ask` stack** `Decomposition(Hybrid(dense))`, so that question returns a balanced 2-Tesla/3-NVIDIA retrieval and a full comparative answer (advanced stage; `notes/advanced/decomposition-notes.md`).
- **Dense retrieval is *blind* to opaque tokens** (the lexical gap). An embedder never learned a useful vector for an acronym/named-act/foreign-entity it didn't see in training (FDDEI, TSMC, GAIN AI Act), so it ranks the answer chunk near the noise floor — not low, *invisible*. No reranker fixes a chunk that was never retrieved. The cure is a parallel lexical (BM25) lane that scores on literal tokens (Experiment 8, hybrid) — now the inner layer of the `ask` stack (`notes/advanced/hybrid-notes.md`).
- **Dense collapses a multi-aspect question onto its dominant aspect** (the enumeration gap). "Revenue *beyond* vehicle sales" is one similarity competition, so the most-similar aspect (used cars) fills the window and energy/leasing/services are starved. No ranker fixes it (a cross-encoder concentrates *harder*); the cure is to *split the question* — and because the aspects are implicit, the split must be **grounded** in a first retrieval pass so an LLM names them from evidence (Experiment 9, retrieve-then-expand) — the outer layer of the `ask` stack (`notes/advanced/enumeration-notes.md`).
...
- **RRF (the textbook hybrid fusion) > plain round-robin interleave** → no: RRF's consensus scoring caps a single-lane answer below any two-lane chunk, so it surfaced *zero* of the opaque-token chunks dense was blind to (lexical hit@5 stuck at the dense baseline). The "simpler" round-robin won.
- **MMR (diversity re-selection) for enumeration** → no: it maximizes *embedding-geometry spread*, which is uncorrelated with the semantic aspects a question enumerates — every Q7 aspect was *in* the pool and MMR still didn't pick them (enumeration 0.12 → 0.12, plus collateral). The deterministic tool was tried first and measured a dead-end.

Two of these wins were cheap and deterministic — **repairing the eval's labels** (a single mislabeled golden answer was inflating a "regression") and **a handful of deterministic lines** (round-robin merge — for cross-company, then hybrid fusion). Repeatedly, "the model is bad" turned out to be "the eval is wrong" — a label error, then a harness bug.

But the spine is **not** "the bigger tool always loses" — that would be the wrong lesson. The enumeration fix (Experiment 9) is the capstone: **grounded** LLM aspect-decomposition was the *first* LLM tool in the whole stage to beat its target (enumeration 0.12 → 0.50, hit@5 0.50 → 1.00). And the telling detail — *the same Haiku model that failed this question **blind** (decomposition Phase B) won it **grounded*** (shown the retrieved chunks first). The model didn't change; what it was given did. So the real discipline: **don't reach for the more powerful tool until a trustworthy measurement says the simpler one isn't enough — and when you do reach for it, the win comes from giving it what it needs (grounding), not from the raw capability. When a result surprises you, suspect the measurement before the model.** Full arc in `notes/advanced/` (`eval-audit.md`, `reranking-results.md`, `decomposition-notes.md`, `hybrid-notes.md`, `enumeration-notes.md`).

*Check yourself:* A SOTA reranker scored near-random and a 10×-pricier decomposer added +0.02 — what's the *first* thing to check, model or measurement, and why? — The same Haiku model lost blind (Phase B) but won grounded (expand) on the *same* question; what does that say about where an LLM's value in a RAG pipeline actually comes from?

### 7. A component's value is context-dependent — measure it in the composition

The sharpest surprise of the advanced stage. Hybrid's **dispatch gate** (engage the BM25 lane only when the query carries an opaque identifier, else pass through to pure dense) measured as a *wash* in isolation — its cross-company gain exactly cancelled an enumeration loss, so the standalone verdict was "redundant, don't ship." That verdict was **wrong**, and only the *composed* measurement showed why. The production retriever is `Decomposition(Hybrid(dense))` — and there the same gate is **load-bearing**: it keeps BM25 out of decomposition's *semantic* comparison sub-queries ("compare supply chain risk…"), where the lexical lane is pure noise that displaces the right chunks. Gated, the composition hits its best cross-company number (0.94) and best overall (recall@5 0.73); ungated, hybrid's collateral leaks into decomposition's branches and drags cross-company down to 0.64.

It happened a *second* time, and more strikingly, when the enumeration layer (expand, Experiment 9) joined the stack. **Q18 — "what does NVIDIA disclose about its reliance on TSMC" — failed in every config built so far**, hybrid included (the full question dilutes "TSMC" with content words, so BM25 ranked the answer chunk only #8). Expand *alone* also failed it (its aspect queries ran through dense, which is blind to "TSMC"). But composed, expand emits *focused* aspect queries ("NVIDIA reliance on TSMC manufacturing") where the opaque token dominates, and routed through hybrid's BM25 lane those rank the answer **#1**. The residual neither tool could fix alone, the two fixed *multiplicatively* — lifting lexical from 0.70 to 0.90 and pushing the stack to hit@5 = 1.00.

The general principle: **you cannot judge a component in isolation when its job is to interact with another.** A part that looks inert alone (the gate) can be load-bearing once composed; two parts that each fail a case (expand and hybrid on TSMC) can solve it together. The only way to know is to measure *in the stack it will actually run in*, not on a bench. (A corollary fell out too: the query-rewriting/splitting layers belong **outermost**, so every downstream lane — including the sparse one — operates on the already-scoped sub-query; the reverse order runs one global BM25 that undoes the split.)

*Check yourself:* The gate was a measured wash on its own — what made it load-bearing inside decomposition? — Expand and hybrid each *failed* the TSMC question alone but *passed* it composed; what does each contribute that the other lacks?
...
- **Experiment 7 — round-robin retrieval** → **shipped (Phase A), recall@5 0.79 → 0.88.** Detect multiple named companies, run one filtered query each, reserve slots, merge. The structural cure for Finding 2. (An LLM-decomposition variant, Phase B/B+, *lost* to it — Principle 6.)
- **Experiment 8 — hybrid retrieval (dense + BM25)** → **shipped, the lexical cure.** A hand-rolled BM25 lane finds the opaque tokens dense is *blind* to (FDDEI, TSMC, GAIN AI Act): lexical recall **0.30 → 0.70**. Required a richer golden set to even *see* (the v1 set had no question dense couldn't answer). Inside it, RRF lost to round-robin interleave (Principle 6) and the dispatch gate proved load-bearing only in composition (Principle 7).
- **Experiment 9 — enumeration via retrieve-then-expand** → **shipped, the last category.** MMR (deterministic diversity) was a measured dead-end; **grounded** LLM aspect-decomposition won (enumeration **0.12 → 0.50**, hit@5 **0.50 → 1.00**) — the first LLM tool in the stage to beat its target, because grounding gave it what blind Phase B lacked (Principle 6).
...
**The full shipped stack** — `Expand(Decomposition(Hybrid(interleave, gated)))` — measures **overall recall@5 0.59 → 0.84, recall@10 0.92, hit@5 0.78 → 1.00** on golden-set v2 (a relevant chunk for *every* question), each category at or near its best at once (cross-company 0.94, lexical 0.90, enumeration 0.50, exact-term 0.92). Lexical reached 0.90 — past hybrid's standalone 0.70 — via the emergent expand×hybrid composition (Principle 7).

**Still queued (hypothesis, not checkbox):**
- **MMR (diversity-aware selection)** to cut the "four adjacent chunks" over-retrieval problem — though note it was *measured a dead-end for enumeration* (Experiment 9); the over-retrieval use is a separate, untested hypothesis.
