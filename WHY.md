...
Hybrid retrieval itself (Experiment 8) *was* a genuine win — a BM25 lane lifted lexical recall 0.30 → 0.70 on questions dense is structurally blind to (acronyms, named acts). But notice *what* won inside it: the cheap, transparent choices. The celebrated fusion (RRF) lost to round-robin; a clever df dispatch gate lost *standalone* too (Principle 7). The two changes that actually moved retrieval across the whole stage were cheap: **repairing the eval's labels** (a single mislabeled golden answer was inflating a "regression" and masquerading as a ~0.05 metric shift) and **a handful of deterministic lines** (round-robin merge — first for cross-company, then for hybrid fusion). Repeatedly, "the model is bad" turned out to be "the eval is wrong" — a label error, then a harness bug. The discipline is the inverse of the usual instinct: **don't reach for the more powerful tool until a trustworthy measurement says the simpler one isn't enough — and when a result surprises you, suspect the measurement before the model.** Full arc in `notes/advanced/` (`eval-audit.md`, `reranking-results.md`, `decomposition-notes.md`, `hybrid-notes.md`).

*Check yourself:* We saw a SOTA reranker score near-random and a 10×-pricier decomposer add only +0.02 — in each case, what's the *first* thing to check, the model or the measurement, and why? — RRF is the standard hybrid fusion everyone reaches for; why did it surface none of the dense-blind answers, and what structural fact about its scoring causes that?
...
The general principle: **you cannot judge a component in isolation when its job is to interact with another.** A part that looks inert alone can be the thing holding the system together once composed — and the only way to know is to measure it *in the stack it will actually run in*, not on a bench. (A corollary fell out too: the query-rewriting/splitting layer belongs **outermost**, so every downstream lane — including the sparse one — operates on the already-scoped sub-query; the reverse order runs one global BM25 that undoes the split.)

*Check yourself:* The gate was a measured wash on its own — what specific interaction made it load-bearing once wrapped inside decomposition? — Why must the splitter sit *outside* hybrid rather than inside it?
...
- **Q7/Q24 aspect-enumeration via retrieve-then-expand** — the last open category (recall ~0.12), and the one decomposition lever left unrealized: ground the split in a first retrieval pass instead of asking a blind LLM to enumerate. Hybrid can't touch it (no opaque token); it's decomposition Phase B's job.
