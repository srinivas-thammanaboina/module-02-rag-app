...
- **Enumeration / retrieve-then-expand (Experiment 9)** — the last red category. **DONE:** MMR (deterministic diversity) was a measured dead-end (geometry ≠ the semantic aspects); **grounded** LLM aspect-decomposition won — enumeration recall@5 **0.12 → 0.50**, hit@5 **0.50 → 1.00**. The first LLM tool in the stage to beat its target (grounding is what blind Phase B lacked). See `enumeration-notes.md`.
- **The full shipped stack** `Expand(Decomposition(Hybrid(interleave, gated)))` — **overall recall@5 0.59 → 0.84, recall@10 0.92, hit@5 0.78 → 1.00** (a relevant chunk for *every* golden question). Per-category bests stack, and expand × hybrid compose *multiplicatively* (Q18 TSMC, dead in every other config, becomes a hit → lexical 0.70 → 0.90). The "measure in composition" lesson, third firing.
